export default async (req: Request) => {
  ...
  try {
    ...
    if (!taskId) {
      return new Response(JSON.stringify({ error: "Missing required parameter: taskId" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    if (status && (status.includes("FAIL") || status.includes("ERROR"))) {
      console.error("Suno generation failed:", JSON.stringify(data.data));
    }
